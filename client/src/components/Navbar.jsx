...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex md:justify-center justify-between items-center p-4">
      ...
      <div className="md:flex-[0.5] flex-initial flex-row justify-center items-center">
        <span className="text-white flex flex-row  justify-center items-center p-2">
          <img
            src={logo}
            alt="logo"
            className="w-16 mx-0 rounded-full p-2 cursor-pointer"
          />
          Transact
        </span>
      </div>
      ...
      <ul className="text-white md:flex hidden list-none flex-row justify-between items-center flex-initial">
        {["Home", "Transactions", "Footer"].map((item, index) => (
